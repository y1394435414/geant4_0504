...
MyRunAction::MyRunAction(){
   ...
   if(MyDetectorConstruction::scintillatorArrangement=="SC"){
        ...
        G4int globalIndex = 0;
        for (G4int groupIndex = 0; groupIndex < MyDetectorConstruction::fGroupCount; groupIndex++) {
            const auto& group = MyDetectorConstruction::fSCArrayGroups[groupIndex];
            for (G4int scintIndex = 0; scintIndex < group.count; scintIndex++) {
                G4String scintillatorName = "G" + G4UIcommand::ConvertToString(groupIndex)
                    + "_S" + G4UIcommand::ConvertToString(scintIndex)
                    + "_" + group.orientation;
                NtupleCreation(scintillatorName, "0", globalIndex * 3);
                NtupleCreation(scintillatorName, "1", globalIndex * 3 + 1);
                NtupleCreation(scintillatorName, "2", globalIndex * 3 + 2);
                globalIndex++;
            }
        }

        man->CreateNtuple("SCARRAY_SecondariesInformation","SCARRAY_SecondariesInformation");
        ...
        man->FinishNtuple(GetSCArrayTotalCount() * 3);
   }
...
void MyRunAction::BeginOfRunAction(const G4Run * run){
   ...
   if(MyDetectorConstruction::scintillatorArrangement=="SCARRAY"){
    file ="SCARRAY_"+ MyDetectorConstruction::scintillatorType
        +"_L"+G4UIcommand::ConvertToString(MyDetectorConstruction::scintillatorLength)+"cm"
        +"_W"+G4UIcommand::ConvertToString(MyDetectorConstruction::scintillatorWidth)+"cm"
        +"_T"+G4UIcommand::ConvertToString(MyDetectorConstruction::scintillatorThickness)+"mm"
        +"_G"+G4UIcommand::ConvertToString(MyDetectorConstruction::fGroupCount)+"groups.root";
   }
