...
The simulation writes ROOT output files. You can draw spectra from those files with:

```bash
root -l -b -q 'plot_energy_spectrum.C("SCARRAY_EJ208_L50cm_W3cm_T30mm_G1groups.root", "edep")'
```

The second argument selects the spectrum:

```bash
"edep"       # primary energy deposition, MeV
"photon"     # optical photon energy, eV
"secondary"  # secondary particle initial energy, MeV
```

Examples:

```bash
# Photon spectrum from one SCARRAY detector tree
root -l -b -q 'plot_energy_spectrum.C("SCARRAY_EJ208_L50cm_W3cm_T30mm_G1groups.root", "photon", "G0_S0_X", 80, 2, 4)'

# Secondary particle spectrum
root -l -b -q 'plot_energy_spectrum.C("SCARRAY_EJ208_L50cm_W3cm_T30mm_G1groups.root", "secondary", "", 100, 0, 1000)'

# Custom output filename
root -l -b -q 'plot_energy_spectrum.C("R_BC404_20mm_2Sensors.root", "edep", "", 60, 0, 12, "edep_spectrum.png")'
```
